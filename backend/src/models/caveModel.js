...
export async function getMapCave(code, name, province, length_min, length_max, depth_min, depth_max, sort, order) {
    // We do this since if user only filters by name and not province query will breaks because province is undefined
    const conditions = [];
    const values = [];
    ...
    const result = await pool.query(
        `SELECT cave_code, cave_name, cave_exploration_status, cave_tags, 
            cave_locality, cave_province, cave_latitude, cave_longitude, 
            cave_depth, cave_length, cave_perspect
         FROM caves ${where} ORDER BY ${sortColumn} ${sortDirection}`,
      values
    );
    return result.rows;
}
